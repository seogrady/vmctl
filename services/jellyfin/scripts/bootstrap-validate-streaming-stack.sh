...
JELLYFIN_BASE_PATH="${JELLYFIN_BASE_URL:-}"
JELLYFIN_BASE_PATH="${JELLYFIN_BASE_PATH%/}"
if [[ "$JELLYFIN_BASE_PATH" == "/" ]]; then
  JELLYFIN_BASE_PATH=""
fi
JELLYFIN_INTERNAL_API="${JELLYFIN_INTERNAL_URL:-http://127.0.0.1:8096}${JELLYFIN_BASE_PATH}"
...
if service_enabled "jellyfin"; then
  ...
  check_http_ok "${JELLYFIN_INTERNAL_API}/System/Info/Public" "jellyfin public info"
  if service_enabled "caddy"; then
    check_http_no_auth "http://127.0.0.1:8097${JELLYFIN_BASE_PATH}/Users/Me" "jellyfin no-login proxy"
...
jellyfin_base_path = (os.environ.get("JELLYFIN_BASE_URL") or "").strip().rstrip("/")
if jellyfin_base_path == "/":
    jellyfin_base_path = ""
jellyfin_internal = urllib.parse.urlparse(os.environ.get("JELLYFIN_INTERNAL_URL", "http://jellyfin:8096"))
jellyfin_url_base = jellyfin_internal.path.rstrip("/") or jellyfin_base_path
if configured_jellyfin and (jellyfin_settings.get("urlBase") or "") != jellyfin_url_base:
    raise SystemExit(
        f"validation failed: Seerr Jellyfin urlBase mismatch: {(jellyfin_settings.get('urlBase') or '')!r} != {jellyfin_url_base!r}"
    )
login_payload = {
...
try:
    ...
    if configured_jellyfin and err.code == 500 and "Jellyfin hostname already configured" in detail:
        pass
    else:
